...
router.post('/', roleGuard(['ADMIN', 'SUPER_ADMIN']), upload.single('image'), designController.createDesign);
router.get('/', designController.getDesigns);
router.get('/:id', designController.getDesignById);
router.put('/:id', roleGuard(['ADMIN', 'SUPER_ADMIN']), upload.single('image'), designController.updateDesign);
